...
user_text = st.text_input("변환할 텍스트", "")  # 기본값 제거

if st.button("미리보기 & 변환"):
    # 점자 전체 길이(mm) 계산
    cells = to_braille_cells(user_text)
    total_length = len(cells) * CELL_PITCH_X
    ...
    # 전체 길이 표시
    st.info(f"📏 점자 전체 길이: {total_length:.1f} mm")
